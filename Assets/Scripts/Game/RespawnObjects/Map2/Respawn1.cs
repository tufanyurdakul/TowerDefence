...
public class Respawn1 : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (respawnTimer <= 0)
        {
            ...
            else if (timer > 160 && timer <=190)
            {
                if (!sendBossVawe1)
                {
                    copyBox = Instantiate(boxBoss1, gameObject.transform.position, gameObject.transform.rotation);
                    sendBossVawe1 = true;
                }
            }
            else if (timer > 190 && timer <= 210)
            {
